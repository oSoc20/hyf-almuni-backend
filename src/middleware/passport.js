...
const alumniStrategy = new JwtStrategy(options, (payload, done)=>{
    // the JWT is valid!
...
// The payload is passed into the verify callback
const companyStrategy = new JwtStrategy(options, (payload, done)=>{
    // the JWT is valid!
    // We will assign the `sub` property on the JWT to the database ID of user
...
module.exports = (passport) => {
    passport.use('alumni-rule', alumniStrategy),
    passport.use('company-rule', companyStrategy)
}